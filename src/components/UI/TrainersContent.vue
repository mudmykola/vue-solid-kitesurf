...
<div class="trainers-content__title " >
  <h2 class="text-38 leading-42 text-center text-c100 font-font-bl pt-20">{{title}}</h2>
  <p class="text-center w-400 mx-auto pt-5 text-16 leading-20 font-font-l">{{desc}} </p>
</div>
  <div class="trainers-content__inner pt-11 ">
    <ul class="grid grid-cols-3 gap-5">
      <li class=" relative cursor-pointer" v-for="item in trainersList" :key="item.id">
        ...
        <p class="absolute bottom-8 left-16 text-default text-24 leading-28 font-font-bl  " >{{item.nameTrainers}}</p>
      </li>
    </ul>
  </div>
</template>
